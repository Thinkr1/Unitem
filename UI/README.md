...
- "Install .app…" + a bundle ID + "Launch" drive `xcrun simctl install` / `simctl launch`, so once you have a real, built `.app` you can push it straight into the booted simulator. Don't have one handy? `../sample-ios/` wraps the repo's actual Login-screen Swift source into a buildable app — run `../sample-ios/run.sh` for the fastest way to see it running for real (or build it once and point the "Install .app…" picker at the resulting `.app`, bundle ID `com.unitem.sample.login`).
